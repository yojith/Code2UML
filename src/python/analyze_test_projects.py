...
def parse_arguments() -> tuple[list[str], bool]:
    parser = argparse.ArgumentParser(description="Analyze Python test projects and print JSON output")
    parser.add_argument(
        "paths",
        nargs="*",
    ...
    return list(args.paths), bool(args.pretty)


def main() -> None:
    paths, pretty = parse_arguments()
    loader = ProjectLoader()
    ast_loader = AbstractSyntaxTreeLoader()
    class_analyzer = ClassAnalyzer()
    relationship_analyzer = RelationshipAnalyzer()

    filepaths = loader.collect_python_files(*paths)
    ast_trees = ast_loader.load(*filepaths)
    diagram = class_analyzer.analyze(ast_trees)
    relationship_analyzer.analyze(ast_trees, diagram)

    payload = {
        "paths": paths,
        "files": filepaths,
        "classes": {
            name: asdict(uml_class)
